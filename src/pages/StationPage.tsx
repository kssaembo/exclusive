...
export function StationPage() {
  ...
  return <main className="app-shell station-page">
    ...
    <section className={`panel trade-flow ${state.phase === 'ended' ? 'station-halted' : ''}`}>
      {state.phase === 'ended' ? <div className="monopoly-stop">{state.endReason === 'monopoly' && <img src={images.ui.monopoly} alt="" />}<span>GAME OVER</span><h2>{state.endReason === 'monopoly' ? '독점이 되었습니다' : '게임이 종료되었습니다'}</h2><p>{state.endReason === 'monopoly' ? '거래소 운영을 중지합니다.' : '더 이상 거래할 수 없습니다.'}</p></div> : <>
