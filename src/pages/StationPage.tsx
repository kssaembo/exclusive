...
export function StationPage() {
  ...
  return <main className="app-shell station-page">
    ...
    <section className={`panel trade-flow ${state.phase === 'ended' ? 'station-halted' : ''}`}>
      {state.phase === 'setup' ? <div className="station-game-wait"><img src={images.ui.station} alt="" /><span>MARKET STANDBY</span><h2>게임 시작을 기다리고 있습니다</h2><p>교사가 학습 인트로와 게임 준비를 마치면<br />이 화면에서 거래를 시작할 수 있습니다.</p><div><i /><b>게임 운영 페이지 연결 유지 중</b></div></div> : state.phase === 'ended' ? <div className="monopoly-stop">{state.endReason === 'monopoly' && <img src={images.ui.monopoly} alt="" />}<span>GAME OVER</span><h2>{state.endReason === 'monopoly' ? '독점이 되었습니다' : '게임이 종료되었습니다'}</h2><p>{state.endReason === 'monopoly' ? '거래소 운영을 중지합니다.' : '더 이상 거래할 수 없습니다.'}</p></div> : <>
    ...
    {pendingPlayer && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="player-confirm-title"><div className="confirm-modal"><span className="label">PLAYER CHECK</span><h2 id="player-confirm-title">{pendingPlayer.name} 본인이 맞나요?</h2><p>공정한 게임을 위해 다른 플레이어의 이름을 선택해서는 안 됩니다.<br />반드시 본인 이름인지 확인해 주세요.</p><div><button onClick={() => setPendingPlayer(null)}>취소</button><button className="primary" onClick={confirmPlayer}>확인하고 들어가기</button></div></div></div>}
