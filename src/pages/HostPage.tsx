...
export function HostPage() {
  ...
  const refreshResources = () => {
    ...
    const refreshedSetup: GameSetup = { playerNames: state.players.map((player) => player.name), durationMinutes: state.settings.durationMinutes }
    engineRef.current.replaceState(createInitialState(refreshedSetup))
    addLog('info', '초기 자원 새로 고침', '플레이어와 제한시간은 유지하고 모든 자원 카드를 다시 배분했습니다.')
  }
  ...
  const requestReconnect = (stationId: string, slot: number) => {
    ...
    addLog(sent ? 'info' : 'warning', `${slot}번 거래소 연결 점검 ${sent ? '요청' : '실패'}`, sent ? '거래소가 연결을 새로 맺습니다. 카드와 게임 상태는 유지됩니다.' : '끊긴 거래소는 태블릿 화면 복귀 시 자동으로 다시 연결됩니다.', stationId)
  }
  ...
  return <main className={`app-shell host-page ${state.resultsRevealed ? 'celebration' : ''}`}>
    ...
    {!state.resultsRevealed ? <section className="host-main-grid"><article className="panel"><div className="section-heading"><div><span className="label">PRIVATE PLAYER STATUS</span><h2>실시간 독점 현황</h2></div><div className="private-heading-actions"><span className="muted">교사 화면 전용 · 거래 성공 {state.trades.filter((trade) => trade.status === 'success').length}건</span>{state.phase === 'setup' && <button className="resource-refresh-button" onClick={refreshResources}>↻ 자원 새로 고침</button>}</div></div><TeamGrid state={state} revealCards /></article><aside className="panel"><div className="section-heading"><div><span className="label">LIVE OPERATIONS</span><h2>운영 기록</h2></div></div><LogPanel logs={logs} /></aside></section>
      : <section className="result-log-layout"><article className="panel"><div className="section-heading"><div><span className="label">GAME OPERATIONS</span><h2>게임 운영 과정 로그</h2></div></div><LogPanel logs={logs} /></article><aside className="panel final-result-panel decorated-results"><div className="section-heading result-heading"><h2>순위 공개</h2></div><p className="score-formula-guide">점수 = 최고 자원 완성률(반올림) − 폭탄 수 × 15점 · 독점 성공자는 1위 고정</p><div className="ranking-table detailed">{state.rankings?.map((rank) => { const rate = Math.round(rank.completionRate * 100); return <div key={rank.playerId}>{rank.rank <= 3 ? <b className={`top-rank rank-${rank.rank}`}>{rank.rank}위</b> : <span className="rank-emblem"><img src={images.ui.rank} alt="" /><b>{rank.rank}</b></span>}<span>{state.players.find((player) => player.id === rank.playerId)?.name}</span><span className="ranking-resource"><img src={resourceIcon(rank.targetType)} alt="" />{rank.targetLabel} {rank.targetCount}/{rank.requiredCount}</span><span>{rate} − ({rank.bombCount}×15)</span><strong>{rank.score}점</strong></div> })}</div></aside></section>}

    <section className="panel diagnostics"><div className="section-heading"><div><span className="label">CONNECTION STATUS</span><h2>거래소 연결 상태</h2></div><span className="muted">연결이 불안정하면 하단의 ‘연결 점검 요청’을 클릭해 보세요. 완전히 끊긴 거래소는 화면 복귀 시 자동 복구됩니다.</span></div><div className="station-list">{[1,2,3].map((slot) => { const station = stations.find((item) => item.slot === slot); return <div className="station-row safe" key={slot}><b>{slot}번 거래소</b>{station ? <><StatusDot status={station.connection} /><span>{station.connection === 'connected' ? station.busy ? '이용 중' : '이용 가능' : '자동 복구 대기'}</span><span>RTT {station.latencyMs ?? '—'}ms</span><button disabled={station.connection !== 'connected'} onClick={() => requestReconnect(station.stationId, slot)}>{station.connection === 'connected' ? '연결 점검 요청' : '자동 복구 대기'}</button></> : <><span className="muted">연결 대기</span><span /><span /><button disabled>연결 대기</button></>}</div> })}</div></section>
  </main>
}
