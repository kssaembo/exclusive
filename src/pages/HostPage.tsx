...
export function HostPage() {
  ...
  return <main className={`app-shell host-page ${state.resultsRevealed ? 'celebration' : ''}`}>
    ...
    <section className="panel diagnostics"><div className="section-heading"><div><span className="label">CONNECTION STATUS</span><h2>거래소 연결 상태</h2></div><span className="muted">연결된 거래소에는 점검 신호를 보냅니다. 끊긴 거래소는 화면 복귀 시 자동 복구됩니다.</span></div><div className="station-list">{[1,2,3].map((slot) => { const station = stations.find((item) => item.slot === slot); return <div className="station-row safe" key={slot}><b>{slot}번 거래소</b>{station ? <><StatusDot status={station.connection} /><span>{station.connection === 'connected' ? station.busy ? '이용 중' : '이용 가능' : '자동 복구 대기'}</span><span>RTT {station.latencyMs ?? '—'}ms</span><button disabled={station.connection !== 'connected'} onClick={() => requestReconnect(station.stationId, slot)}>{station.connection === 'connected' ? '연결 점검 요청' : '자동 복구 대기'}</button></> : <><span className="muted">연결 대기</span><span /><span /><button disabled>연결 대기</button></>}</div> })}</div></section>
  </main>
}
