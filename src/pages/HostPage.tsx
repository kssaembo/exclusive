...
export function HostPage() {
  ...
  return <main className={`app-shell host-page ${state.resultsRevealed ? 'celebration' : ''}`}>
    ...
    <section className="host-command panel">
      <div className="command-room"><span>ROOM CODE</span><div className="room-code-with-qr"><strong>{roomCode}</strong>{qrUrl && <button className="qr-button framed-qr" onClick={() => setQrOpen(true)} aria-label="거래소 QR 크게 보기"><img className="qr-frame-art" src={images.ui.qrFrame} alt="" /><img className="host-qr" src={qrUrl} alt="거래소 접속 QR" /></button>}</div><button onClick={() => navigator.clipboard.writeText(stationUrl)}>거래소 주소 복사</button></div>
