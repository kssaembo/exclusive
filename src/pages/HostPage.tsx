...
const isCurrentState = (value: GameState | null): value is GameState => !!value && Array.isArray(value.players) && typeof value.gameId === 'string' && value.settings?.playerCount === 8
...
export function HostPage() {
  ...
  useEffect(() => {
    ...
    function handleNetworkMessage(stationId: string, message: WireMessage) {
      if (message.type === 'AUTH_REQUEST') {
        ...
        network.send(player ? { type: 'AUTH_RESULT', requestId: message.requestId, ok: true, player } : { type: 'AUTH_RESULT', requestId: message.requestId, ok: false, message: '인증코드 또는 게임 상태를 확인하세요.' }, stationId)
        addLog(player ? 'success' : 'warning', `참가자 인증 ${player ? '성공' : '실패'}`, player?.name, stationId)
      } else if (message.type === 'TRADE_REQUEST') {
        engineRef.current.execute(message.request).then((result) => { network.send({ type: 'TRADE_RESULT', result }, stationId); const current = engineRef.current.getState(); const a = current.players.find((p) => p.id === message.request.playerAId)?.name; const b = current.players.find((p) => p.id === message.request.playerBId)?.name; addLog(result.ok ? 'success' : 'error', `거래 ${result.ok ? '완료' : '실패'}`, `${a} ↔ ${b} · ${message.request.playerACardIds.length}장 · ${resultText(result)}`, stationId) })
        ...
        if (claim) { network.send({ type: 'CLAIM_RECEIVED', claim }, stationId); const current = engineRef.current.getState(); addLog('warning', '독점 선언 접수', `${current.players.find((p) => p.id === claim.playerId)?.name} · ${MONOPOLY_RULES.find((r) => r.type === claim.resourceType)?.label}`, stationId) }
  ...
  const makeRequest = (a: number, b: number, tradeId = createId('host-test')): TradeRequest => { const snapshot = engineRef.current.getState(); const pa = snapshot.players[a]; const pb = snapshot.players[b]; const aa = engineRef.current.authenticate(pa.accessCode, 'host-test')!; const ab = engineRef.current.authenticate(pb.accessCode, 'host-test')!; return { tradeId, stationId: 'host-test', playerAId: pa.id, playerBId: pb.id, playerACardIds: [pa.cards[0].id], playerBCardIds: [pb.cards[0].id], playerAAuthToken: aa.authToken, playerBAuthToken: ab.authToken, expectedPlayerVersions: { [pa.id]: pa.version, [pb.id]: pb.version }, processingDelayMs: 300 } }
  ...
  return <main className="app-shell host-page">
    ...
    <section className="host-command panel"><div className="command-room"><span>ROOM CODE</span><strong>{roomCode}</strong><button onClick={() => navigator.clipboard.writeText(stationUrl)}>거래소 주소 복사</button></div><div className="command-timer"><span>{state.phase === 'setup' ? '게임 준비' : state.phase === 'active' ? '남은 시간' : '게임 종료'}</span><strong>{formatTime(remainingMs)}</strong><div><button onClick={openBoard}>학생 전광판 열기 ↗</button>{state.phase === 'setup' && <button className="primary" onClick={startGame}>게임 시작</button>}{state.phase === 'active' && <button className="danger-text" onClick={endGame}>수동 종료</button>}{state.phase === 'ended' && <button onClick={() => navigate('/setup')}>새 게임 설정</button>}</div></div><div className="command-stations"><span>거래소 연결</span><strong>{connected}<small>/3</small></strong><div className="station-lights">{[1,2,3].map((slot) => { const station = stations.find((s) => s.slot === slot); return <i className={station?.connection === 'connected' ? 'on' : ''} title={`거래소 ${slot}`} key={slot} /> })}</div></div>{qrUrl && <img className="host-qr" src={qrUrl} alt="거래소 접속 QR" />}</section>

    {pendingClaims.length > 0 && <section className="panel claim-alert"><div><span className="label">MONOPOLY CLAIM</span><h2>독점 선언을 검증하세요</h2></div>{pendingClaims.map((claim) => <div className="claim-row" key={claim.claimId}><span><b>{state.players.find((p) => p.id === claim.playerId)?.name}</b> · {MONOPOLY_RULES.find((r) => r.type === claim.resourceType)?.label} 독점</span><div><button className="primary" onClick={() => resolveClaim(claim.claimId, true)}>카드 확인 후 승인</button><button className="danger-text" onClick={() => resolveClaim(claim.claimId, false)}>반려</button></div></div>)}</section>}

    {state.phase === 'ended' && <section className="panel result-banner"><span>FINAL RESULT</span><h2>{winner ? `${winner.name} 우승` : '게임 종료'}</h2><p>{state.endReason === 'monopoly' ? `${MONOPOLY_RULES.find((r) => r.type === state.winnerResourceType)?.label} 독점 성공` : '종료 시점의 자원 완성도에서 폭탄 감점을 반영한 결과입니다.'}</p><button onClick={downloadLog}>운영 로그 CSV 내려받기</button></section>}

    <section className="host-main-grid"><article className="panel"><div className="section-heading"><div><span className="label">PRIVATE PLAYER STATUS</span><h2>실시간 독점 현황</h2></div><span className="muted">교사 화면 전용 · 거래 성공 {state.trades.filter((t) => t.status === 'success').length}건</span></div><TeamGrid state={state} revealCards />{state.rankings && <div className="ranking-table">{state.rankings.map((rank) => <div key={rank.playerId}><b>{rank.rank}위</b><span>{state.players.find((p) => p.id === rank.playerId)?.name}</span><span>{rank.targetLabel} {rank.targetCount}/{rank.requiredCount}</span><span>폭탄 {rank.bombCount}</span><strong>{rank.score}점</strong></div>)}</div>}</article><aside className="panel"><div className="section-heading"><div><span className="label">LIVE OPERATIONS</span><h2>운영 기록</h2></div>{state.phase === 'ended' && <button onClick={downloadLog}>CSV</button>}</div><LogPanel logs={logs} /></aside></section>
