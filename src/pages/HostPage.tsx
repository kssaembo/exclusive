...
export function HostPage() {
  ...
  return <main className={`app-shell host-page ${state.resultsRevealed ? 'celebration' : ''}`}>
    ...
    <header className="topbar"><div><p className="eyebrow">TEACHER CONTROL ROOM</p><h1>독점게임 운영 페이지</h1></div><div className="header-status"><StatusDot status={hostReady ? 'connected' : 'connecting'} /><button className="rules-modal-button" onClick={() => setRulesOpen(true)}>게임 규칙</button></div></header>
    <section className="host-command panel">
      <div className="command-room"><span>ROOM CODE</span><div className="room-code-with-qr"><strong>{roomCode}</strong>{qrUrl && <button className="qr-button framed-qr" onClick={() => setQrOpen(true)} aria-label="거래소 QR 크게 보기"><img className="qr-frame-art" src={images.ui.qrFrame} alt="" /><img className="host-qr" src={qrUrl} alt="거래소 접속 QR" /></button>}<button className="copy-station-url" onClick={() => navigator.clipboard.writeText(stationUrl)}>거래소 주소 복사</button></div><p className="room-operation-guide">실제 독점 게임 운영을 위한 거래소를 활성화해 주세요. 3개까지 거래소를 열 수 있습니다(2개 추천). 학급 사정에 맞게 태블릿으로 QR코드를 인식한 후 게임 공간과 별도의 공간에 거래소를 설치해 주세요. 예: 앞문·뒷문 앞에 책상 설치</p></div>
