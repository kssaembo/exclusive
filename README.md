...
- PC에서 관리자 탭 등 다른 화면으로 이동해도 정상 WebRTC 연결 유지
- 화면 복귀·온라인 복구 시 실제로 끊긴 경우에만 자동 재접속

## v1.0.1 연결 안정화 수정

- `visibilitychange` 발생 시 정상 연결을 강제로 닫던 조건 오류를 수정했습니다.
- 탭이 숨겨진 동안에는 재접속을 시도하지 않습니다.
- 탭 복귀, `online`, `pageshow` 이벤트에서는 온라인이면서 실제 연결이 끊어진 경우에만 복구합니다.
- 연결 시도 중에는 추가 연결을 만들지 않아 중복 재접속을 방지합니다.
- 네트워크 진단 도구의 `수동 재연결`은 의도적인 연결 재생성 기능으로 그대로 유지됩니다.
